...
export default function FAQ() {
  ...
  return (
    <section className="py-24 relative bg-white border-y border-[#C99C3D]/10" id="faq">
      <div className="max-w-4xl mx-auto px-6">
        ...
        >
          <h2 className="text-sm text-orange-600 uppercase tracking-[0.2em] mb-4">Clarity</h2>
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <motion.div 
              ...
              className="border border-[#C99C3D]/20 bg-white rounded-2xl overflow-hidden"
            >
              <button
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className="w-full flex items-center justify-between p-6 text-left focus:outline-none"
              >
                <span className="text-lg font-sans text-[#2C241B] pr-8">{faq.question}</span>
              ...
              <AnimatePresence>
                {openIndex === index && (
                  ...
                  >
                    <div className="px-6 pb-6 text-[#5C4B3D] font-light leading-relaxed">
                      {faq.answer}
                    </div>
                  </motion.div>
                )}
